...
object SQLArrays extends App {
  ...
  try { sql"select array['foo',NULL,'baz']".query[List[String]].quick.unsafeRunSync() }
  catch { case e: Throwable => println(e.toString) }
  // throws:
  ...
  try { sql"select array['foo',NULL,'baz']".query[Option[List[String]]].quick.unsafeRunSync() }
  catch { case e: Throwable => println(e.toString) }
  // throws:
  // [error] (run-main-50) doobie.util.invariant$NullableCellRead$:
  // SQL `NULL` appears in an array cell that was asserted to be non-null.
