...
object CustomMappings extends App {
  ...
  s"${dash(5)} Deriving Get and Put from Existing Instances ${dash(5)}".yellow.println()

  // If we don’t have the Get or Put instance we need, we can often make one from an existing instance.
  // Consider here a type Nat of natural numbers, along with a conversion to and from Int.
  ...
  s"${dash(5)} Deriving Get and Put from Meta ${dash(5)}".yellow.println()
  ...
  s"${dash(5)} Defining Get and Put for Exotic Types ${dash(5)}".yellow.println()
  ...
  s"${dash(5)} Deriving Read and Write from Existing Instances ${dash(5)}".yellow.println()
